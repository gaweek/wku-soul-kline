...
const ModeSegment: React.FC<{
  ...
  <div
    className="wku-lens-grid"
    role="tablist"
    aria-label="切换 WKU 分析模式"
  >
    {modeTabs.map((item) => {
      ...
      return (
        <button
          ...
          className={`wku-lens-card ${active ? 'is-active' : ''}`}
        >
          <span className="flex items-start justify-between gap-3">
            <span>
              <span className="block text-base font-black leading-5">{item.title}</span>
              <span className="mt-1 block text-xs font-black">{item.subtitle}</span>
            </span>
            <MiniKLine active={active} match={item.value === 'match'} />
          </span>
          <span className="mt-3 block text-xs font-semibold leading-5">{item.body}</span>
          ...
          <div className="wku-workbench wku-mode-bar">
            ...
            <div className="wku-workbench-body">
              <aside className="wku-lens-desk">
                <div>
                  <p className="text-xs font-black text-teal-700">WKU 读盘镜头</p>
                  <h3 className="mt-1 text-xl font-black text-slate-950">先选一条要读的 K 线</h3>
              ...
              <section className="wku-input-deck">
                <div className="wku-input-deck-head">
                  <div>
                    ...
                    <h3 className="mt-1 text-xl font-black text-slate-950">{mode === 'single' ? '把你的真实社交样本放进来' : '把两个人的样本并排放进来'}</h3>
