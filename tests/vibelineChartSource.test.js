...
test('match mode supports a two-person invite link flow', () => {
  ...
  assert.match(pageSource, /createShortInviteLink/);
  assert.match(pageSource, /fetchShortInvitePayload/);
  assert.match(pageSource, /wkuInvite/);
  assert.match(pageSource, /const buildShortInviteHref = \(id: string\) => `\/#\/i\/\$\{id\}`/);
  assert.match(pageSource, /getHashPayload\(hashRoute, 'i'\)/);
  assert.match(pageSource, /activeShortInviteId/);
  assert.match(pageSource, /onCopyInvite/);
  assert.match(pageSource, /inviteLinkPending/);
  ...
  assert.match(pageSource, /setPersonA\(activeInvitePayload\.personA\)/);
  assert.match(pageSource, /setPersonB\(normalizeInviteProfile\(\)\)/);
  assert.match(cssSource, /wku-invite-panel/);
  assert.equal(pageSource.includes('return `${window.location.origin}${buildInviteHref(payload)}`'), false);
